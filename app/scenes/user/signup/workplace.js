...
const scene = new BaseScene('workplace')

scene.enter( async ctx => {
     const LANG = ctx.session.LANG
     const caption = Functions.getText(LANG, 'user.signup.workplace.header')
...
scene.on('text', async ctx => {
     if(ctx.message.entities && ctx.message.entities[0].type == 'bot_command' ){
          ...
          ctx.session.signup_user.workplace = ctx.message.text
