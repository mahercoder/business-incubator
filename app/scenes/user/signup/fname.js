...
const scene = new BaseScene('fname')

scene.enter( async ctx => {
     const LANG = ctx.session.LANG
     const caption = Functions.getText(LANG, 'user.signup.fname.header')
     ...
     ctx.replyWithHTML("Assalomu alaykum botga xush kelibsiz!")
     .then( () => {
...
scene.on('text', async ctx => {
     if(ctx.message.entities && ctx.message.entities[0].type == 'bot_command' ){
          ...
          ctx.session.signup_user.fname = ctx.message.text
